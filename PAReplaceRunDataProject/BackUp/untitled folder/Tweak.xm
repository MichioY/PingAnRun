...
- (long long)integratedSteps{
  ...
  NSInteger tempSteps = [KKPARunConfig sharedConfig].steps;

  long long steps = tempSteps;
...
- (long long)heartRate{
  ...
  NSInteger tempHeartRates = [KKPARunConfig sharedConfig].heartRate;

  long long heartRate = tempHeartRates;

  return heartRate;
}
%end

// %hook PARSHealthCircleWalkingManager
//
